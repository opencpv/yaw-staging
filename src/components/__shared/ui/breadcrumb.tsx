...
>(({ className, ...props }, ref) => (
  <ol
    ref={ref}
    className={cn(
      "flex flex-wrap items-center gap-1.5 break-words text-base text-shade-200 sm:gap-2.5",
      className,
    )}
    {...props}
  />
...
const BaseBreadcrumbLink = React.forwardRef<
  HTMLAnchorElement,
...
>(({ asChild, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ref={ref}
      className={cn(
        "border-b border-accent transition-colors hover:text-shade-500",
        className,
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <span
    ...
    aria-disabled="true"
    aria-current="page"
    className={cn("font-medium text-shade-500", className)}
    {...props}
  />
));
