...
export const POST = async (req: NextRequest) => {
  ...
  try {
    const customSourceHeader = req.headers.get("X-Custom-Source");

    if (customSourceHeader === "rr-app") {
      appBody = await req.json();
    } else {
      body = await req.json();
      if (body.action === "Meeting.scheduled") {
        await supabase.from("agent_request_matches").upsert({
          id: appBody!.matchId || 0,
        ...
        await supabase
          ...
          .match({ id: appBody!.matchId || 0, meeting_id: body.entity.id });
