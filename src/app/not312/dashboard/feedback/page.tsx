...
const PageView = () => {
  ...
  const ExportFilterButton = () => (
    <Dropdown>
      <DropdownTrigger>
        <Button className="border-2 border-accent-200">
          {capitalizeName(selectedExportValue)}
        </Button>
      </DropdownTrigger>
      <DropdownMenu
        aria-label="feedback filter"
        ...
        selectionMode="single"
        items={exportType}
        selectedKeys={selectedExportTypeKey}
        onSelectionChange={setSelectedExportTypeKey}
      >
        {(item: any) => (
          <DropdownItem
            key={item.key}
