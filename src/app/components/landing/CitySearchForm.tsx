...
const CitySearchForm = ({className}: {className?: string;}) => {
  return (
    <div
      className={`flex h-32 w-full items-center justify-center bg-primary-500 bg-opacity-80 sm:h-52 md:w-full 2k:h-64 ${styles.rectangle} ${className}`}
    >
      <form className="flex h-16 w-[75%] translate-x-[-10%]">
        {/* <AutoComplete
          data={cities}
        ...
        /> */}
        <input
          className="h-full w-full flex-[4] rounded-l-xl rounded-r-none bg-secondary-300 px-4 py-3 text-sm font-[600] text-neutral-900 placeholder:text-neutral-400 focus:outline-none xs:flex-[5] xs:text-2xl"
          placeholder="Madina, Accra"
        />
