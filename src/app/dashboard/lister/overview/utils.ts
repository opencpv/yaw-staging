...
export const getFormValues = (values: typeof ListingDefaultValues) => {
 ...
 const agentFee = convertYesNoToBoolean(values?.require_agent_fee) ? Number(values?.agent_fee) : null
  const viewingFee = convertYesNoToBoolean(values?.require_viewing_fee) ? Number(values?.viewing_fee) : null
  const refundableSecurityDeposit = convertYesNoToBoolean(values?.require_refundable_security_deposit) ? Number(values?.refundable_security_deposit) : null

  const additionalFees = convertYesNoToBoolean(values?.require_additional_fees) ? values?.additional_fees : null
